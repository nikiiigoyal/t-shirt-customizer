...
{snap.intro ? (
  <motion.section key='main' {...config}>
    <div className="section--container">
      {/* seperate animation for this title */}
      ...
      {/* sepreate animaion for para */}
      <div className="support--container">
    <motion.div key='p' initial={{y: 100,opacity:0}}
    animate={{y: 0, opacity:1}}
    transition={{type: 'spring',damping: 7, stiffness:30, restDelta: 0.001, duration:0.6,delay:0.2,delayChildren:0.2}}
    >
  <p>Create your unique and exclusive shirt with our brand-new 3D customization tool. <strong>Unleash your imagination</strong> and define your
                    own style.</p>
                    {/* button */}
<button>CUSTOMIZE IT
...
return (
  <div className="customizer">
    <div className="color-options">
      {/* loop thriugh each color in colors array */}
      {snap.colors.map ((color) => (
  <div key={color} className={`circle`} style={{background:color}} onClick={() => (state.color = color)}></div>

      ))}
    </div>
  {/* decals picker this chnages logo */}
  <div className="declas">
    <div className="decals-container">
      {snap.decals.map((decal) => (
        <div key={decal} className={`decal`} onClick={() => (state.decal = decal)}>
          <img src={decal + '_thumb.png'} alt="brand"></img>
        </div>
      ))}
    </div>
  </div>
  <button className="share" style={{background: snap.color}} onClick={() => {
    const link = document.createElement('a')
    link.setAttribute('download','canvs.png')
